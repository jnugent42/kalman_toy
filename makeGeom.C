void makeGeom()
{
  ...
  //   gSystem->Load("libGeom");
   ...
   for (int i=0; i<20; ++i){
     ...
     /* top->AddNode(redBullCan, i, new TGeoTranslation(-219+(distance*i),0,0)); */
   ...
   for (int i=0; i<2; ++i){
     TGeoVolume *greenBullCan = gGeoManager->MakeBox("greenBullCan", vacuum, thickness, 100, 100);//, 90., 270.);
     greenBullCan->SetLineColor(kGreen);
     /* top->AddNode(blueBullCan, i, new TGeoTranslation(-217+(distance*i),0,0)); */
     /* top->AddNode(greenBullCan, i, new TGeoTranslation(-100+(distance*i),0,0)); */
   }
